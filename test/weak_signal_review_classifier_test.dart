...
void main() {
  group('WeakSignalReviewClassifier', () {
    ...
    test('does not classify weak subscription reminder without lifecycle risk',
        () {
    ...
    test(
        'does not classify generic recurring process wording without trusted lifecycle risk',
        () {
      final result = classifier.classify(
    ...
    test('does not classify membership renewal reminder without risk language',
        () {
      final result = classifier.classify(
    ...
    test(
        'does not classify upcoming payment reminder without failure or cancellation risk',
        () {
      final result = classifier.classify(
    ...
    test(
        'does not classify generic app-store recurring wording without lifecycle risk',
        () {
      final result = classifier.classify(
        message(
    ...
    test('classifies subscription suspended as unknownReview', () {
      final result = classifier.classify(
        message(
            'Your Netflix subscription has been suspended due to payment failure.'),
      );
      ...
      expect(
          result.evidenceFragments
              .any((f) => f.note?.contains('suspension') ?? false),
          isTrue);
    });
    ...
    test('classifies free trial ending as unknownReview', () {
      final result = classifier.classify(
        message(
            'Your Google One free trial ends in 3 days. Renew now to keep your 100 GB storage.'),
      );
      ...
      expect(
          result.evidenceFragments
              .any((f) => f.note?.contains('trial ending') ?? false),
          isTrue);
    });
    ...
    test('classifies subscription expiring as unknownReview', () {
      ...
      expect(
          result.evidenceFragments
              .any((f) => f.note?.contains('expiry') ?? false),
          isTrue);
    });
    ...
    test('does not classify loan suspension noise', () {
      final result = classifier.classify(
        message(
            'Your Personal Loan account has been suspended. Please contact branch.'),
      );
    ...
    test('does not classify credit card expiry noise', () {
      final result = classifier.classify(
        message(
            'Your SBI Credit Card XX4321 expires on 05/29. Request a new one in the app.'),
      );
